...
//!
//! ```text
//! cargo run --release --example send_colorbar -- [name] [options]
//!   --width N --height N --fps N --animate|--no-animate
//!   --tone-hz F --rate N --channels N --audio-samples N
//!   --profile lq|sq|hq   (default: sq)
//!   --send-queue N       (default: 4; 0 = blocking)
//!   --send-buffer N --recv-buffer N
//! ```
    ...
    while running.load(Ordering::Relaxed) {
        {
            ...
            let want_video = s.video_subscribed();
            let want_audio = s.audio_subscribed();
...
fn audio_loop(
    ...
    let samples = opts.audio_samples.max(1) as i64;
    let rate = opts.sample_rate.max(1) as i64;
    let mut samples_sent = 0i64;

    while running.load(Ordering::Relaxed) {
        let want = sender.lock().map(|s| s.audio_subscribed()).unwrap_or(false);
        if !want {
